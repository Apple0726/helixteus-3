...
func collect_rsrc(tile, tile_id:int):
	...
	match tile.tile_str:
		"ME":
			...
			tile.stored = Helper.add_minerals(stored)
		...
		if tile.type == "plant":
			...
			if tile.is_constructing:
				...
				if tile.has("rover_id"):
					...
					tile.erase("rover_id")
				hboxes[id2].get_node("Path1").text = String(tile.path_1)
				if tile.has("path_2"):
					hboxes[id2].get_node("Path2").text = String(tile.path_2)
				if tile.has("path_3"):
		...
		"ME", "PP", "MM":
			...
			if stored < cap:
				...
				if c_t - c_d > prod:
					...
					tile.stored += rsrc_num
					tile.collect_date += prod * rsrc_num
					if tile.stored >= tile.path_2_value:
						tile.stored = tile.path_2_value
						current_bar.value = 0
