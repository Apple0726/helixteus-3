...
#				debris.scale = Vector2.ONE * (pow(rng.randf(), 4) / 2.0 + 0.2)
			...
			if level > 0:
				...
				if rand < ch:
					...
					if met_spawned != "":
						...
						add_child(deposit)
						deposit.position = cave_wall.map_to_local(Vector2(i, j))
		...
		for room in rooms:
			...
			for tile in room.tiles:
				...
				if rand < formula:
					...
					chest.scale *= 0.8
					chest.position = cave_BG.map_to_local(get_tile_pos(tile)) + Vector2(100, 100)
	...
	if tile.cave.has("special_cave") and tile.cave.special_cave == 5 and cave_floor == 3:
		...
		relic.position = cave_BG.map_to_local(get_tile_pos(relic_tile)) + Vector2(100, 100)
		add_child(relic)
	...
	if cave_floor == num_floors and not boss_cave:
		...
		wormhole.position = cave_BG.map_to_local(get_tile_pos(wormhole_tile)) + Vector2(100, 100)
		add_child(wormhole)
...
func mine_wall(item:Dictionary, _tile_highlight, delta):
	...
	if not tiles_touched_by_laser.has(st):
		...
		add_child(sq_bar)
		sq_bar.position = cave_BG.map_to_local(get_tile_pos(tile_highlighted_for_mining))
		tile.bar = sq_bar
	if st != "-1":
