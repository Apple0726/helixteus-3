...
func _ready():
	...
	ray_length = 1200.0
	idle_move_speed = 250.0
	atk_move_speed = 600.0
...
func on_time_out():
	...
	if (sees_player or is_aggr()) and counter < 5:
		for i in range(0, 3):
			cave_ref.add_proj(true, pr.position, 12.0, rot + i * 2*PI/3, load("res://Graphics/Cave/Projectiles/enemy_bullet.png"), atk * 2.0)
	counter += 1
	if counter >= 15:
		counter = 0
		rot = rand_range(0, 2 * PI / 3)
