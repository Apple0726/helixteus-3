...
func _ready():
	...
	if ResourceLoader.exists(dir_str):
		$Sprite2D.texture = load(dir_str)
	else:
		$Sprite2D.texture = load("res://icon.png")

func connect_events(dialogue_id:int, dialogue_ref):
	if is_connected("body_entered",Callable(self,"_on_NPC_body_entered")):
		disconnect("body_entered",Callable(self,"_on_NPC_body_entered"))
	connect("body_entered",Callable(self,"_on_NPC_body_entered").bind(dialogue_id, dialogue_ref))
	if not is_connected("body_exited",Callable(self,"_on_NPC_body_exited")):
		connect("body_exited",Callable(self,"_on_NPC_body_exited").bind(dialogue_ref))
