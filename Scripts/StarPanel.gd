		...
		if game.system_data[game.c_s].has("conquered"):
			...
			if star.has("MS_lv"):
				if star.MS_lv == Data.MS_num_stages[star.MS]:
					construct_btn.mouse_entered.connect(game.show_tooltip.bind(tr("MS_FULLY_CONSTRUCTED")))
					construct_btn.disabled = true
				elif not game.science_unlocked.has("{name}{stage}".format({"name":star.MS, "stage":star.MS_lv + 1})):
...
func zoom_to_star(star:Dictionary):
	if game.bottom_info_action in ["building_DS", "building_CBS", "building_PK", "building_MB"]:
		return
	if not game.view.obj.MS_constr_data.is_empty():
		return
	if tween:
		tween.kill()
	tween = create_tween()
