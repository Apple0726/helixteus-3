	...
	if ok:
		for prop in $TP/VBox.get_children():
			...
			if prop.name == "time_speed" and game.subject_levels.dimensional_power >= 4:
				update_cave_time_speed_text()
			if prop.has_node("HSlider"):
				prop.get_node("HSlider").min_value = game.physics_bonus.MVOUP
				prop.get_node("HSlider").max_value = ceil(init_PP * PP_multiplier / Data.univ_prop_weights[prop.name])
...
func update_cave_time_speed_text():
	var cave_time_speed:float = Helper.get_logarithmic_time_speed(game.subject_levels.dimensional_power, $TP/VBox/time_speed/HSlider.value)
	if is_equal_approx(cave_time_speed, 1.0):
		$TP/VBox/time_speed/Unit.text = ""
	else:
		$TP/VBox/time_speed/Unit.text = " (%s)" % [tr("TIME_SPEED_IN_CAVES") % cave_time_speed]
