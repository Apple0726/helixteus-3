...
func update_cone(_emission_cone_angle_visual: float):
	...
	for i in range(-100, 100):
		...
		while true:
			...
			add_weapon_data(hit_target, angle, inner_light)
		...
		if inner_light:
			inner_light_polygon.append(hit_point)
	light_polygon.append(Vector2.ZERO)
	inner_light_polygon.append(Vector2.ZERO)
	$LightPolygon.polygon = light_polygon
	$InnerLightPolygon.polygon = inner_light_polygon

func add_weapon_data(hit_target, angle:float, inner_light:bool):
	var has_shader = hit_target.has_node("Sprite2D") and hit_target.get_node("Sprite2D").material
	if hit_target in weapon_datas:
		var additional_damage = damage / 200.0
		if inner_light:
			additional_damage *= 3.0
		weapon_datas[hit_target].damage += additional_damage
		...
		if shooter.type == Battle.EntityType.SHIP:
			if inner_light:
				weapon_data.damage *= 3.0
				weapon_data.buffs.accuracy *= 2
			if shooter.light_levels[0] >= 4:
				weapon_data.status_effects[Battle.StatusEffect.EXPOSED] = 2.0 if inner_light else 1.0
			if shooter.light_levels[0] >= 2:
				weapon_data.buffs.attack = -2 if inner_light else -1
...
func fire_light(fade_delay: float):
	...
	$LightPolygon.color.a = 1.0
	light_anim.tween_property($LightPolygon, "modulate:a", 0.0, fade_delay)
