...
func _on_btn_pressed(_tab:int):
	...
	if tab == OVERCLOCK:
		for item_id in [Item.OVERCLOCK1, Item.OVERCLOCK2, Item.OVERCLOCK3, Item.OVERCLOCK4, Item.OVERCLOCK5, Item.OVERCLOCK6]:
			...
			item.get_node("TextureButton").mouse_entered.connect(Callable(game, "show_tooltip").bind(tooltip_txt, {"imgs": tooltip_icons}))
	...
	if game.check_enough(item_total_costs):
		if tab == PICKAXE:
			if game.pickaxe.has("name"):
				game.show_YN_panel(buy_pickaxe.bind(item_total_costs), tr("REPLACE_PICKAXE") % [tr(game.pickaxe.name.to_upper()), tr(item_selected.id.to_upper())])
			else:
