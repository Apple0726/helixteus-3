...
func refresh_overlay():
	$Panel/HBoxContainer/OptionButton.clear()
	match game.c_v:
		"galaxy":
			$Panel/HBoxContainer/OptionButton.add_item(tr("NUMBER_OF_PLANETS"))
			$Panel/HBoxContainer/OptionButton.add_item(tr("SYSTEM_ENTERED"))
			$Panel/HBoxContainer/OptionButton.selected = game.overlay_data.galaxy.overlay
			send_overlay_info(game.overlay_data.galaxy.overlay)
			$Panel/HBoxContainer/CheckBox.pressed = game.overlay_data.galaxy.visible
