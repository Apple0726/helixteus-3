...
func _ready():
	...
	for i in 4:
		if i >= len(battle_scene.ship_data):
			...
			XP_bar_tween.tween_property(get_node("Ship%s/XP/TextureProgressGained" % (i + 1)), "value", battle_scene.ship_data[i].XP + battle_scene.XP_earned, 1.0).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_CUBIC)
