...
func _ready():
	...
	for i in 4:
		if i >= len(battle_scene.ship_data):
			...
			create_tween().tween_property(get_node("Ship%s/XP" % (i + 1)), "modulate:a", 1.0, 0.2).set_delay(i * 0.2 + 0.2)
...
func _process(delta):
	for i in len(battle_scene.ship_data):
		...
		XP_node.value = move_toward(XP_node.value, XP_node.value + battle_scene.XP_earned, (battle_scene.XP_earned + battle_scene.ship_data[i].XP - XP_node.value) * delta * 2)
