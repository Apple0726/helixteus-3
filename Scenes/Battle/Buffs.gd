...
func update():
	...
	$Attack/Label.text = "{sign}{buff}".format({"sign":"+" if entity.attack_buff > 0 else "", "buff":Helper.clever_snap(entity.attack_buff)})
	...
	$Defense/Label.text = "{sign}{buff}".format({"sign":"+" if entity.defense_buff > 0 else "", "buff":Helper.clever_snap(entity.defense_buff)})
	...
	$Accuracy/Label.text = "{sign}{buff}".format({"sign":"+" if entity.accuracy_buff > 0 else "", "buff":Helper.clever_snap(entity.accuracy_buff)})
	...
	$Agility/Label.text = "{sign}{buff}".format({"sign":"+" if entity.agility_buff > 0 else "", "buff":Helper.clever_snap(entity.agility_buff)})
